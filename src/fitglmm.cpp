 ...
 *  Copyright (C) 2014,2015  Han Chen, Matthew P. Conomos
 *
...
double Loglikelihood2 (void *params)
{
	...
	for(i=1; i<=q; ++i) {
	        stringstream kins;
		kins << "kins" << i;
		Sigma = Sigma + tau[i-1] * symmatl(as<mat>(par->Phi[kins.str()]));
	}
...
extern "C" 
{
  SEXP fitglmm_brent(SEXP Y_in, SEXP X_in, SEXP Phi_in, SEXP sqrtW_in, SEXP method_in, SEXP dispersion_in, SEXP tau_in, SEXP fixtau_in, SEXP maxiter_in, SEXP tol_in, SEXP taumin_in, SEXP taumax_in, SEXP tauregion_in)
{
	try {
		vec Y = as<vec>(Y_in);
		mat X = as<mat>(X_in);
		mat Phi = as<mat>(Phi_in);
		vec sqrtW = as<vec>(sqrtW_in);
		...
		Y %= sqrtW;
		X.each_col() %= sqrtW;
		Phi = diagmat(sqrtW) * Phi * diagmat(sqrtW);
		mat U;
		vec eval;
  ...
  SEXP fitglmm_nm(SEXP Y_in, SEXP X_in, SEXP q_in, SEXP Phi_in, SEXP W_in, SEXP method_in, SEXP dispersion_in, SEXP tau_in, SEXP fixtau_in, SEXP maxiter_in, SEXP tol_in)
{
  ...
  SEXP fitglmm_ai(SEXP Y_in, SEXP X_in, SEXP q_in, SEXP Phi_in, SEXP W_in, SEXP tau_in, SEXP fixtau_in, SEXP maxiter_in, SEXP tol_in)
{
	try {
		vec Y = as<vec>(Y_in);
		mat X = as<mat>(X_in);
		...
		vec alpha(p), eta(n);
		cube PHI(n, n, q);
		mat Sigma = tau[0] * diagmat(1.0 / W);
		for(size_t i=1; i<=q; ++i) {
		        stringstream kins;
			kins << "kins" << i;
			PHI.slice(i-1) = symmatl(as<mat>(Phi[kins.str()]));
			Sigma = Sigma + tau[i] * PHI.slice(i-1);
		}
		...
		eta = Y - tau[0] * (Sigma_i * (Y - X * alpha)) / W;
		if(q2 > 0) {
		        const uvec idxtau = find(fixtau == 0);
		        mat AI(q2, q2);
		        vec PY = P * Y;
			...
			for(size_t i=0; i<q2; ++i) {
			        if(i == 0 && idxtau[0] == 0) {
				        PAPY = PY / W;
					score[0] = dot(PAPY, PY) - sum(diagvec(P) / W);
...
SEXP glmm_score_bed(SEXP res_in, SEXP P_in, SEXP bimfile_in, SEXP bedfile_in, SEXP outfile_in, SEXP center_in, SEXP miss_method_in, SEXP nperbatch_in, SEXP select_in) {
	try {
		vec res = as<vec>(res_in);
		mat P = symmatl(as<mat>(P_in));
		const char center = Rcpp::as<char>(center_in);
